...
- 你可以将插件发布到 Jenkins 项目的 Maven 仓库，它是 Jenkins 项目使用的更新站点的数据来源。
...
然后在第一行进行简短的总结，并在随后的行中提供更多详细信息。如：
```
[JENKINS-00000] Frobnicate the widget
```

如果给定的提交修复了指定的问题，
那么使用以下前缀之一将会自动化解决相关的 JIRA 问题。
```
...
因此需要在 GitHub 上有一个用户帐户，并且需要有一个公共仓库来存放插件源码。

为了完整的发布你的插件，需要注册一个 [Jenkins 社区帐号](https://accounts.jenkins.io/)，
...
如果审查人员要求你更改，请按照要求进行更改。

笔者的申请为：[HOSTING-750](https://issues.jenkins-ci.org/browse/HOSTING-750)，
...
### 创建 wiki 页面
尽管对发布插件来说这不是严格要求的，但最好为插件创建一个 wiki 页面来存储文档。关于如何执行此操作的详细信息，请参阅[插件 wiki 页面指南](https://jenkins.io/doc/developer/publishing/wiki-page)。

笔者所写的插件的 wiki 页面为：[Maven SNAPSHOT Check Plugin](https://wiki.jenkins.io/display/JENKINS/Maven+SNAPSHOT+Check+Plugin) 。
其间除了官方文档，笔者也参考了其它插件 wiki 页面的排版。
...
### 申请上传权限
在源码被 fork 到 `jenkinsci` 组织后，需要提交一个权限上传请求。
...
在创建 PR 后，会有说明以及 checklist 让提交人对该 PR 进行确认。  
...
当执行 release 时，Maven Release Plugin 会自动往仓库推送，
...
### 为插件分类
在 Jenkins [插件列表页](https://plugins.jenkins.io/)，可以对插件进行分类显示。
